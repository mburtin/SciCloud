      ...
      <div class="w-full max-w-md">
        ...
        <div v-else-if="!hasUsers && !showCreateAdmin" class="bg-white rounded-lg p-8 shadow-sm">
          ...
          <div class="space-y-4 mb-8">
            <div class="flex items-start space-x-3">
              ...
              <div>
                <h3 class="font-medium text-gray-900">{{ t('welcome.features.projectManagement.title') }}</h3>
                <p class="text-sm text-gray-600">{{ t('welcome.features.projectManagement.description') }}</p>
              </div>
            </div>
            <div class="flex items-start space-x-3">
              <div class="flex-shrink-0 w-6 h-6 bg-green-100 rounded-full flex items-center justify-center mt-0.5">
              ...
              <div>
                <h3 class="font-medium text-gray-900">{{ t('welcome.features.equipmentTracking.title') }}</h3>
                <p class="text-sm text-gray-600">{{ t('welcome.features.equipmentTracking.description') }}</p>
              </div>
            </div>
            <div class="flex items-start space-x-3">
              <div class="flex-shrink-0 w-6 h-6 bg-green-100 rounded-full flex items-center justify-center mt-0.5">
              ...
              <div>
                <h3 class="font-medium text-gray-900">{{ t('welcome.features.teamCollaboration.title') }}</h3>
                <p class="text-sm text-gray-600">{{ t('welcome.features.teamCollaboration.description') }}</p>
              </div>
            </div>
          </div>
        ...
        <div v-else-if="showSuccess" class="bg-white rounded-lg p-8 shadow-sm text-center">
          ...
          <p class="text-gray-600 mb-8">
            {{ t('welcome.setupCompleteDescription') }}
          </p>
        ...
        <div v-else-if="showCreateAdmin" class="bg-white rounded-lg p-8 shadow-sm">
          ...
          <div class="text-center mb-8">
            ...
            <h1 class="text-2xl font-medium text-gray-900 mb-2">
              {{ t('welcome.adminForm.title') }}
            </h1>
            <p class="text-gray-600 text-sm">
              {{ t('welcome.adminForm.subtitle') }}
            </p>
          </div>
          ...
          <form class="space-y-4" @submit.prevent="handleCreateAdmin">
            <div class="grid grid-cols-2 gap-4">
              <div>
                <Label for="firstName" class="text-gray-700">{{ t('welcome.adminForm.firstName') }}</Label>
                <Input id="firstName" v-model="adminForm.firstName" type="text" required placeholder="John"
                  class="mt-1" />
              </div>
              <div>
                <Label for="lastName" class="text-gray-700">{{ t('welcome.adminForm.lastName') }}</Label>
                <Input id="lastName" v-model="adminForm.lastName" type="text" required placeholder="Doe" class="mt-1" />
              </div>
            </div>

            <div>
              <Label for="email" class="text-gray-700">{{ t('welcome.adminForm.email') }}</Label>
              <Input id="email" v-model="adminForm.email" type="email" required placeholder="admin@laboratory.com"
                class="mt-1" />
            </div>

            <div>
              <Label for="password" class="text-gray-700">{{ t('welcome.adminForm.password') }}</Label>
              <Input id="password" v-model="adminForm.password" type="password" required placeholder="••••••••"
                class="mt-1" />
              <p class="text-xs text-gray-500 mt-1">
                {{ t('welcome.adminForm.passwordRequirement') }}
              </p>
            </div>

            <div>
              <Label for="confirmPassword" class="text-gray-700">{{ t('welcome.adminForm.confirmPassword') }}</Label>
            ...
            <Button type="submit" class="w-full mt-6" :disabled="isCreatingAdmin">
              <span v-if="isCreatingAdmin" class="flex items-center">
                ...
                {{ t('welcome.adminForm.creating') }}
              </span>
              <span v-else>{{ t('welcome.adminForm.createAccount') }}</span>
            </Button>
          </form>

          <div class="text-center mt-6">
            <p class="text-sm text-gray-500">
              {{ t('welcome.adminForm.fullAccess') }}
            </p>
          </div>
        </div>
...
const checkUsersExist = async () => {
  try {
    ...
    error.value = t('common.messages.networkError');
...
const handleCreateAdmin = async () => {
  ...
  if (adminForm.value.password.length < 8) {
    error.value = t('common.validation.minLength', { min: 8 });
    return;
  }
  ...
  try {
    ...
    if (result.success) {
      ...
      error.value = result.error || t('common.messages.error');
    }
  } catch (err) {
    error.value = t('common.messages.error');
